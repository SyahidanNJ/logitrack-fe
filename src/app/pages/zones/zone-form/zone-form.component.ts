...
export class ZoneFormComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      ...
      type: ['storage'] // default type
    });

    this.api.get('warehouses').subscribe(res => {
      this.warehouses = res.data || [];
    });
  }

  loadItem(): void {
    this.api.get(`zones/${this.itemId}`).subscribe({
  ...
  onSubmit(): void {
    ...
    const request$ = this.isEdit ? this.api.put(`zones/${this.itemId}`, payload) : this.api.post('zones', payload);
